...
SELECT count(libname) --counts all values in the 2014 table's libname column
FROM pls_fy2014_pupld14a;


SELECT count(DISTINCT libname) --counts all values in the 2014 table's libname column but includes DISTINCT
FROM pls_fy2014_pupld14a;

--(listing 8.6) Finding the most and fewest visits using max() and min()

SELECT max(visits), min(visits) --gives us min and max values, although still needs modifying
FROM pls_fy2014_pupld14a;
